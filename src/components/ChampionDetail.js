...
const ChampionDetail = () => {
  ...
  useEffect(() => {
    async function fetchChampionData() {
      ...
      try {
        ...
        if (!champResponse.ok)
          throw new Error(
            `Champion data fetch error: ${champResponse.status}. \n Champion info not added yet by Riot Games`,
          );
        ...
        if (champData) {
          ...
          if (!skinResponse.ok)
            throw new Error(
              `Skin data fetch error: ${skinResponse.status}. \n Champion info not added yet by Riot Games.`,
            );
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 bg-white bg-opacity-50 p-6 rounded-lg mx-4 md:mx-8 mt-8">
        <div className="flex flex-col md:flex-row gap-4 md:gap-8">
          ...
          <div className="flex-1">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold italic text-yellow-400">
              {champion.name}
            </h1>
            ...
            <p className="text-lg mt-2">
              <strong>Class:</strong> {champion.tag.join(", ")}
            </p>
      ...
      <div className="bg-white bg-opacity-50 p-4 rounded-lg mx-4 md:mx-8 mt-8 shadow-lg">
        ...
        <div className="flex overflow-x-auto space-x-4 pb-4">
          {skins.map((skin) => (
            <div
              key={skin.id}
              className="relative w-48 h-72 bg-white shadow-lg rounded-lg overflow-hidden flex-shrink-0"
            >
              <img
                alt={skin.name}
      ...
      <p className="my-2 text-center text-2xl text-white">
        {" "}
